# Kimi Code – Inspect SWE

## Overview

The `kimi_code()` agent uses the unattended mode of Moonshot AI [Kimi Code](https://github.com/MoonshotAI/kimi-code) to execute agentic tasks within the Inspect sandbox. Model API calls that occur in the sandbox are proxied back to Inspect for handling by the model provider for the current task.

> **NOTE: NoteKimi Code Installation**
>
> By default, the agent will download the current stable version of Kimi Code and copy it to the sandbox. You can also exercise more explicit control over which version of Kimi Code is used—see the [Installation](#installation) section below for details.
...
``` bash
inspect eval ctf.py --solver inspect_swe/kimi_code
```
...
| `centaur` | Run in [Centaur Mode](#centaur-mode), which makes Kimi Code available to an Inspect `human_cli()` agent rather than running it unattended. |
...
| `disallowed_tools` | Tool names to deny via Kimi Code permission rules. |
| `cwd` | Working directory for Kimi Code session. |
| `env` | Environment variables to set for Kimi Code. |
| `version` | Version of Kimi Code to use (see [Installation](#installation) below for details) |
...
| Option | Description |
|----|----|
| `"auto"` | Use any available version of Kimi Code in the sandbox, otherwise download the latest version. |
| `"sandbox"` | Use the version of Kimi Code in the sandbox (raises `RuntimeError` if not available in the sandbox) |
...
1.  Pre-install the version of Kimi Code you want to use in the sandbox, then use `version="sandbox"`:

    ``` python
    kimi_code(version="sandbox")
    ```

2.  Download the version of Kimi Code you want to use into the cache, then specify that version explicitly:

    ``` python
    # download the agent binary during installation/configuration
    download_agent_binary("kimi_code", "0.29.0", "linux-x64")

    # reference that version in your task (no download will occur)
    kimi_code(version="0.29.0")
    ```
...
``` bash
inspect eval ctf.py --solver inspect_swe/kimi_code -S centaur=true
```
