...
def check_system_explorer_example(
    ...
    # run_example fails the test on a task-level failure, an errored sample, a
    # sample cut short by a limit, or a sample with no agent turn. What it
    # cannot know is that this example defines a scorer (model_graded_qa), so
    # a run that got all the way through must carry a score. Assert that, and
    # not the grade itself: the examples exist to exercise the agents, not to
    # measure how well they explore.
