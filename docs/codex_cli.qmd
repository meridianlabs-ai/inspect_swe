...
When `bwrap` is unavailable — the case for most sandbox images out of the box — `auto_review` still functions, but through a model-driven path rather than OS-enforced sandboxing. Codex cannot start its sandbox, so each command it attempts to sandbox fails with a `bubblewrap is unavailable` error that is returned to the model as the command's output; **Codex does not automatically fall back to a review-only mode.** A capable model recovers by re-issuing the command with escalated permissions (`require_escalated`) — which the guardian adjudicates and, if approved, runs outside the sandbox — and typically carries that escalation forward, so the guardian still reviews what the agent does. The caveats of this path: it costs a failed attempt on the first command (and again on any later command the model runs without escalation, which re-triggers the sandbox error), and review depends on the model choosing to escalate rather than being enforced by Codex. Whether or not the sandbox is available, the guardian may deny an escalation, and repeated denials interrupt the turn.
