...
- `seccomp=unconfined` removes the outer Inspect container's syscall filtering. For security-sensitive deployments, a tailored seccomp profile that allowlists only the syscalls bubblewrap needs (`unshare`, `clone`, `mount`, `pivot_root`, `umount2`, `setns`) is a stricter alternative to disabling seccomp wholesale.
...
- You should not need `systempaths=unconfined`: Codex detects a masked `/proc` and retries without mounting a fresh procfs.
