...
| `approve_static_mcp_tools` | Also auto-approve write-type tool calls on caller-supplied (static) MCP servers, not just bridged ones. Defaults to `False`. Takes effect only when the effective approval policy is `"never"`, so it does nothing under `auto_review`. |
