...
| Option | Description |
|------------------------------------|------------------------------------|
| `system_prompt` | System prompt that replaces Claude Code's built-in prompt. |
| `append_system_prompt` | Additional instructions to append to the effective system prompt. |
...
For example, here we replace Claude Code's built-in system prompt and disallow the `{{< meta agent_disallowed_tool >}}` tool:
...
To preserve Claude Code's built-in system prompt and add instructions, use `append_system_prompt`:

```python
{{< meta agent >}}(
    append_system_prompt="You are an ace system researcher.",
)
```
