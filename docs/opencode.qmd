...
| `session_title` | Fixed title for the {{< meta agent_name >}} session (default `"Inspect eval"`). A fixed title makes {{< meta agent_name >}} skip its automatic title-generation model call; pass `None` to restore it. |
...
::: callout-note
The task prompt is delivered to `opencode run` on stdin, which requires {{< meta agent_name >}} 1.14.42 or later (released May 2026). Earlier versions prepend a newline to piped input, and for very short prompts (under 20 characters) that can keep the agent's main conversation from being recognised in the Inspect transcript. This only matters when an older version is already installed in your sandbox image and picked up via `"auto"` or `"sandbox"`; downloaded versions are always recent enough.
:::

{{< include _snippets/agent_installation.md >}}

{{< include _snippets/agent_binary_provenance.md >}}
